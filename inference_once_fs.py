...
class LocalDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, image_root, transform, clip_precessor):
    ...
    def __getitem__(self, idx):
        ...
        image_clip = self.clip_processor.preprocess(image_np, return_tensors="pt")["pixel_values"][0]
...
def main(args):
    ...
    dataset = LocalDataset(input_dir, transform, clip_image_processor)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=8)


    from concurrent.futures import ThreadPoolExecutor

    def save_figs(save_img_path, image_np, save_mask_path, pred_mask, save_vis_path):
        ...
        cv2.imwrite(save_vis_path, save_img)

    with ThreadPoolExecutor(max_workers=8) as executor:
        for item in tqdm(dataloader):
            image_src, image, image_clip, resize_list, original_size_list, image_name = item
            image_np = image_src[0].numpy()
            image, image_clip = image.cuda(), image_clip.cuda()
            ...
            save_img_path = os.path.join(img_out_dir, f"{image_name}.png")
            save_mask_path = os.path.join(mask_out_dir, f"{image_name}.jpg")
            save_vis_path = os.path.join(vis_out_dir, f"{image_name}.png")
            executor.submit(save_figs, save_img_path, image_np, save_mask_path, pred_mask, save_vis_path)

            # save_path = f"{img_out_dir}/{image_name}.png"
            # cv2.imwrite(save_path, image_np)
            # print(f"{save_path} has been saved.")

            # save_path = f"{mask_out_dir}/{image_name}.jpg"
